...
# save unified CSV
points_df = hcat(points_df, values)
CSV.write("test_datasets/tab_df.csv", points_df)
...
μₖ, σ²ₖ = simple_kriging_omnihorizontal(xyz, points_no3, NO3, gaussian3D, p_no3, minimum(NO3), (minimum(NO3)*0.1)^2, 2000., 20, 80)
μₖ[μₖ .< 0] .= minimum(NO3)
...
μₖ, σ²ₖ = simple_kriging_omnihorizontal(xyz, points_al, Al, gaussian3D, p_al, minimum(Al), var(Al), 2000., 20, 80)
μₖ[μₖ .< 0] .= minimum(Al)
...
μₖ, σ²ₖ = simple_kriging_omnihorizontal(xyz, points_fe, Fe, gaussian3D, p_fe, minimum(Fe), var(Fe), 2000., 20, 80)
μₖ[μₖ .< 0] .= minimum(Fe)
...
μₖ, σ²ₖ = simple_kriging_omnihorizontal(xyz, points_f, F, gaussian3D, p_f, minimum(F), var(F), 2000., 20, 80)
μₖ[μₖ .< 0] .= minimum(F)
...
μₖ, σ²ₖ = ordinary_kriging_omnihorizontal(xyz, points_se, Se, gaussian3D, p_se, minimum(Se), var(Se), 2000., 10, 80)
μₖ[μₖ .< 0] .= minimum(Se)
...
μₖ, σ²ₖ = ordinary_kriging_omnihorizontal(xyz, points_co, Co, gaussian3D, p_co, minimum(Co), var(Co), 2000., 10, 80)
μₖ[μₖ .< 0] .= minimum(Co)
...
μₖ, σ²ₖ = simple_kriging_omnihorizontal(xyz, points_sv, svoc, gaussian3D, p_sv, minimum(svoc), var(svoc), 2000., 20, 80)
